...
// 690 x 80 ms = 55.2 s (~60 s once sent to emoncms).
...
// RFM69CW settings.
#define NETWORK_NODE_ID       25  // 0-31
...
void loop()
{
  ...
  taransay_th.supply_voltage = battery_voltage;

  // Read from SI7021 temperature and humidity sensor.
  if (si7021_enabled) {
    si7021_read();
    taransay_th.temperature = int(si7021_temperature * 0.1);
    taransay_th.humidity = int(si7021_humidity * 0.1);
  }
  ...
  taransay_th.ext_temperature = ds18b20_temperature;
  ...
  // Send payload.
  rf12_sendNow(0, &taransay_th, sizeof taransay_th);
